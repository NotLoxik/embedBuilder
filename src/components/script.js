...
let params = new URLSearchParams(location.search),
  ...
  noUser =
    localStorage.getItem("noUser") || hasParam("nouser") || options.noUser,
  onlyEmbed = hasParam("embed") || options.onlyEmbed,
  allowPlaceholders = hasParam("placeholders") || options.allowPlaceholders,
  autoUpdateURL =
    localStorage.getItem("autoUpdateURL") || options.autoUpdateURL,
  noMultiEmbedsOption =
    localStorage.getItem("noMultiEmbedsOption") ||
    hasParam("nomultiembedsoption") ||
    options.noMultiEmbedsOption,
  single = noMultiEmbedsOption
    ? options.single ?? true
  ...
  multiEmbeds = !single,
  autoParams =
    localStorage.getItem("autoParams") ||
    hasParam("autoparams") ||
    options.autoParams,
  hideEditor =
    ...
    options.hideEditor,
  hidePreview =
    localStorage.getItem("hidepreview") ||
    hasParam("hidepreview") ||
    options.hidePreview,
  hideMenu =
    localStorage.getItem("hideMenu") ||
    hasParam("hidemenu") ||
    options.hideMenu,
  sourceOption =
    localStorage.getItem("sourceOption") ||
    hasParam("sourceoption") ||
...
const guiEmbedIndex = (guiEl) => {
  ...
  return !gui
    ? -1
    : Array.from(gui.querySelectorAll(".guiEmbed")).indexOf(guiEmbed);
};
...
const encodeJson = (jsonCode, withURL = false, redirect = false) => {
  let data = btoa(
    encodeURIComponent(
      JSON.stringify(typeof jsonCode === "object" ? jsonCode : json)
    )
  );
...
let jsonToBase64 = encodeJson,
  base64ToJson = decodeJson;
...
const reverse = (reversed, callback) => {
  ...
  if (side.nextElementSibling)
    side.parentElement.insertBefore(side.nextElementSibling, side);
  else
    side.parentElement.insertBefore(side, side.parentElement.firstElementChild);

  const isReversed = document.body.classList.toggle("reversed");
  if (autoParams)
    isReversed
      ? urlOptions({ set: ["reverse", ""] })
      : urlOptions({ remove: "reverse" });
};
...
const indexOfEmptyGuiEmbed = (text) => {
  for (const [i, element] of document
    .querySelectorAll(".msgEmbed>.container .embed")
    .entries())
...
const changeLastActiveGuiEmbed = (index) => {
  const pickerEmbedText = document.querySelector(
    ".colors .cTop .embedText>span"
  );
  ...
  if (pickerEmbedText) {
    ...
    pickerEmbedText.onclick = () => {
      const newIndex = parseInt(
        prompt(
          "Enter an embed number" +
            (guiEmbedNames.length > 1 ? `, 1 - ${guiEmbedNames.length}` : ""),
          index + 1
        )
      );
      ...
      if (newIndex < 1 || newIndex > guiEmbedNames.length)
        return error(
          guiEmbedNames.length === 1
            ? `'${newIndex}' is not a valid embed number`
            : `'${newIndex}' doesn't seem like a number between 1 and ${guiEmbedNames.length}`
        );
...
const afterBuilding = () =>
  autoUpdateURL && urlOptions({ set: ["data", encodeJson(json)] });
// Parses emojis to images and adds code highlighting.
const externalParsing = ({ noEmojis, element } = {}) => {
  !noEmojis &&
    twemoji.parse(element || document.querySelector(".msgEmbed"), {
...
let jsonObject = window.json || {
  ...
  embed: {
    ...
    image: {
      url: "https://raw.githubusercontent.com/notloxik/embedBuilder/main/src/assets/img/banner.png",
    },
    footer: {
      text: "Footer text",
      icon_url: "https://cdn.discordapp.com/embed/avatars/0.png",
...
addEventListener("DOMContentLoaded", () => {
  ...
  if (noMultiEmbedsOption) document.querySelector(".box .item.multi")?.remove();
  if (inIframe)
  ...
  if (single) {
    ...
    if (autoParams)
      single
        ? urlOptions({ set: ["single", ""] })
        : urlOptions({ remove: "single" });
  }
  ...
  if (noUser) {
    ...
    if (autoParams)
      noUser
        ? urlOptions({ set: ["nouser", ""] })
        : urlOptions({ remove: "nouser" });
  } else {
  ...
  for (const e of document.querySelectorAll(".clickable > img"))
    e.parentElement.addEventListener("mouseup", (el) =>
      window.open(el.target.src)
    );
  ...
  error = (msg, time = "5s") => {
    ...
    if (!notif.style.display) return (notif.style.display = "block"), false;
  ...
  const allGood = (embedObj) => {
    ...
    if (
      embedObj.timestamp &&
      new Date(embedObj.timestamp).toString() === "Invalid Date"
    ) {
  ...
  const markup = (txt, { replaceEmojis, inlineBlock, inEmbed }) => {
    if (replaceEmojis)
      txt = txt.replace(
        /(?<!code(?: \w+=".+")?>[^>]+)(?<!\/[^\s"]+?):((?!\/)\w+):/g,
        (match, p) => (p && emojis[p] ? emojis[p] : match)
      );
    ...
    txt = txt
      ...
      .replace(
        /&#60;#\d+&#62;/g,
        () => `<span class="mention channel interactive">channel</span>`
      )
      .replace(/&#60;@(?:&#38;|!)?\d+&#62;|@(?:everyone|here)/g, (match) => {
        if (match.startsWith("@"))
          return `<span class="mention">${match}</span>`;
        else
          return `<span class="mention interactive">@${
            match.includes("&#38;") ? "role" : "user"
          }</span>`;
      });
      ...
      txt = txt.replace(
        ...
        (m, w, x) => {
          if (w) return `<pre><code class="${w}">${x.trim()}</code></pre>`;
            ...
            return `<pre><code class="hljs nohighlight">${x.trim()}</code></pre>`;
        }
      );
      // Inline code
  ...
  const createEmbedFields = (fields, embedFields) => {
    ...
    for (const [i, f] of fields.entries()) {
      if (f.name && f.value) {
        const fieldElement = embedFields.insertBefore(
        ...
        if (
          (fields[i].inline &&
            ...
            ((i === 0 && fields[i + 2] && !fields[i + 2].inline) || // it's not the first field in the embed but the previous field is not inline or -
              (((i > 0 && !fields[i - 1].inline) ||
                ...
                (i >= 3 &&
                  ...
                  fields[i - 2].inline &&
                  fields[i - 3].inline &&
                  (fields[i - 4] ? !fields[i - 4].inline : !fields[i - 4]))) &&
                ...
                (i == fields.length - 2 || !fields[i + 2].inline)))) ||
          (i % 3 === 0 && i == fields.length - 2)
        ) {
        ...
        if (!f.inline)
          ...
          fieldElement.outerHTML = `
                        <div class="embedField ${num}${
            gridCol ? " colNum-2" : ""
          }" style="grid-column: ${gridCol || colNum + " / " + (colNum + 4)};">
                            <div class="embedFieldName">${markup(
                              encodeHTML(f.name),
                              {
                                inEmbed: true,
    ...
    for (const e of document.querySelectorAll(
      '.embedField[style="grid-column: 1 / 5;"]'
    ))
      if (
        !e.nextElementSibling ||
        e.nextElementSibling.style.gridColumn === "1 / 13"
      )
  ...
  const timestamp = (stringISO) => {
    ...
    return today.toDateString() === date.toDateString()
      ...
      : tommorrow.toDateString() === date.toDateString()
      ? `Tomorrow at ${dateArray}`
  ...
  const [guiFragment, fieldFragment, embedFragment, guiEmbedAddFragment] =
    Array.from({ length: 4 }, () => document.createDocumentFragment());
  embedFragment.appendChild(
    document.querySelector(".embed.markup").cloneNode(true)
  );
  guiEmbedAddFragment.appendChild(
    document.querySelector(".guiEmbedAdd").cloneNode(true)
  );
  fieldFragment.appendChild(
    document.querySelector(".edit>.fields>.field").cloneNode(true)
  );
  ...
  buildGui = (object = jsonObject, opts) => {
    gui.innerHTML = "";
    gui
      .appendChild(guiEmbedAddFragment.firstChild.cloneNode(true))
    ...
    for (const child of Array.from(guiFragment.childNodes)) {
      if (child.classList?.[1] === "content")
        gui.insertBefore(
          gui.appendChild(child.cloneNode(true)),
          gui.appendChild(child.nextElementSibling.cloneNode(true))
        ).nextElementSibling.firstElementChild.value = object.content || "";
      else if (child.classList?.[1] === "guiEmbedName") {
        for (const [i, embed] of (object.embeds.length
          ? object.embeds
          : [{}]
        ).entries()) {
          const guiEmbedName = gui.appendChild(child.cloneNode(true));

          guiEmbedName.querySelector(".text").innerHTML = `Embed ${i + 1}${
          ...
          const guiEmbed = gui.appendChild(
            createElement({ div: { className: "guiEmbed" } })
          );
            ...
            if (!child2?.classList.contains("edit")) {
              ...
              switch (child2.classList[1]) {
                case "author":
                  ...
                  if (authorURL)
                    edit.querySelector(".imgParent").style.content =
                      "url(" + encodeHTML(authorURL) + ")";
                  edit.querySelector(".editAuthorLink").value = authorURL;
                  edit.querySelector(".editAuthorName").value =
                    embed?.author?.name || "";
                  break;
                case "title":
                  ...
                  break;
                case "description":
                  edit.querySelector(".editDescription").value =
                    embed?.description || "";
                  break;
                case "thumbnail":
                  ...
                  for (const f of embed?.fields || []) {
                    ...
                    for (const child of Array.from(
                      ...
                      if (child.classList.contains("inlineCheck"))
                        ...
                        f.value &&
                        child.classList?.contains("fieldInner")
                      )
                        (newChild.querySelector(
                          ".designerFieldName input"
                        ).value = f.name || ""),
                          (newChild.querySelector(
      ...
      e.addEventListener("click", (el) => {
        if (e?.classList.contains("active"))
          ...
          if (e.classList.contains("guiEmbedName"))
            ...
            const date = new Date(
              jsonObject.embeds[guiEmbedIndex(e)]?.timestamp || new Date()
            );
            const textElement = e.nextElementSibling.querySelector("svg>text");
            const dateInput = textElement
              .closest(".footerDate")
              .querySelector("input");
    ...
    const lastTabs = Array.from(
      document.querySelectorAll(".footer.rows2, .image.largeImg")
    );
    const requiresView = matchMedia(
      `${smallerScreen.media}, (max-height: 845px)`
    );
    const addGuiEventListeners = () => {
        ...
        e.onclick = () => {
          if (lastTabs.includes(e) || requiresView.matches) {
            if (!reverseColumns || !smallerScreen.matches)
              ...
              e.nextElementSibling.classList.contains("edit") &&
      ...
      for (const e of document.querySelectorAll(".addField"))
        e.onclick = () => {
          ...
          const indexOfGuiEmbed = Array.from(
            gui.querySelectorAll(".guiEmbed")
          ).indexOf(guiEmbed);
          if (indexOfGuiEmbed === -1)
            return error("Could not find the embed to add the field to.");

          const fieldsObj = ((jsonObject.embeds[indexOfGuiEmbed] ??=
            {}).fields ??= []);
          if (fieldsObj.length >= 25)
            return error("Cannot have more than 25 fields");
          fieldsObj.push({
            name: "Field name",
            value: "Field value",
            inline: false,
          ...
          const newField = guiEmbed
            ?.querySelector(".item.fields+.edit>.fields")
            ?.appendChild(fieldFragment.firstChild.cloneNode(true));
          ...
          if (!smallerScreen.matches) {
            const firstFieldInput = newField.querySelector(
              ".designerFieldName input"
            );

            firstFieldInput?.setSelectionRange(
              firstFieldInput.value.length,
              firstFieldInput.value.length
            );
      ...
      for (const e of document.querySelectorAll(".fields .field .removeBtn"))
        e.onclick = () => {
          ...
          const fieldIndex = Array.from(e.closest(".fields").children).indexOf(
            e.closest(".field")
          );
      ...
      for (const e of gui.querySelectorAll("textarea, input"))
        e.oninput = (el) => {
          ...
          if (field) {
            ...
            const embedFields = document
              .querySelectorAll(".container>.embed")
              [index]?.querySelector(".embedFields");
            ...
            switch (el.target.classList?.[0]) {
              ...
              case "editTitle":
                ...
                const guiEmbedName =
                  el.target.closest(".guiEmbed")?.previousElementSibling;
                if (guiEmbedName?.classList.contains("guiEmbedName"))
                  guiEmbedName.querySelector(".text").innerHTML = `${
                    guiEmbedName.innerText.split(":")[0]
              ...
              case "embedFooterTimestamp":
                ...
                el.target.parentElement.querySelector("svg>text").textContent =
            ...
            const nonEmptyEmbedObjects = json.embeds?.filter(
              (o) => 0 in Object.keys(o)
            );
          ...
          document
            .querySelectorAll(".msgEmbed>.container")
            [guiEmbedIndex(el.target)]?.querySelector(".emptyEmbed")
            ?.classList.remove("emptyEmbed");
        };
        ...
        browse.onclick = (e) => {
          ...
          fileInput.onchange = (el) => {
            if (el.target.files[0].size > 32 * 1024 * 1024)
              return uploadError(
                "File is too large. Maximum size is 32 MB.",
                browse,
                5000
              );
            ...
            formData.append(
              "key",
              options.uploadKey || "93385e22b0619db73a5525140b13491c"
            ); // Add your own key through the uploadKey option.
            ...
            fetch("https://api.imgbb.com/1/upload", {
              ...
              .then((res) => {
                ...
                if (!res.success) {
                  console.log(
                    "Upload failed:",
                    res.data?.error || res.error?.message || res
                  );
                  return uploadError(
                    res.data?.error ||
                      res.error?.message ||
                      "Request failed. (Check dev-console)",
                    browse
                  );
                }

                imgSrc(
                  edit.querySelector(".editIcon > .imgParent"),
                  res.data.url
                );
                const linkInput = edit.querySelector("input[type=text]");
                const textInput = edit.querySelector(
                  "input[class$=Name], input[class$=Text]"
                );
                ...
                console.info(
                  `${res.data.url} will be deleted in ${
                    expiration / 60 / 60
                  } hours. To delete it now, visit ${
                    res.data.delete_url
                  } and scroll down to find the delete button.`
                );
    ...
    if (opts?.guiEmbedIndex) {
      activeGuiEmbed = Array.from(
        document.querySelectorAll(".gui .item.guiEmbedName")
      )[opts.guiEmbedIndex];
    ...
    if (opts?.activateClassNames)
      ...
      const tabs = (opts.guiTabs.split?.(/, */) || opts.guiTabs).filter(
        (item) => item
      );
  ...
  buildEmbed = ({ jsonData, only, index = 0 } = {}) => {
    ...
    try {
      ...
      switch (only) {
                ...
                `${
                  embedObj.url
                    ? '<a class="anchor" target="_blank" href="' +
                      encodeHTML(url(embedObj.url)) +
                      '">' +
                      encodeHTML(embedObj.title) +
                      "</a>"
                    : encodeHTML(embedObj.title)
                }`,
              ...
              `
                        ${
                          embedObj.author.icon_url
                            ? '<img class="embedAuthorIcon embedAuthorLink" src="' +
                              encodeHTML(url(embedObj.author.icon_url)) +
                              '">'
                            : ""
                        }
                        ${
                          embedObj.author.url
                            ? '<a class="embedAuthorNameLink embedLink embedAuthorName" href="' +
                              encodeHTML(url(embedObj.author.url)) +
                              '" target="_blank">' +
                              encodeHTML(embedObj.author.name) +
                              "</a>"
        ...
        case "embedDescription":
          ...
          if (!embedObj.description) hide(embedDescription);
            ...
            display(
              embedDescription,
              markup(encodeHTML(embedObj.description), {
                inEmbed: true,
                replaceEmojis: true,
              })
            );
        ...
        case "embedThumbnail":
          const embedThumbnailLink = embed?.querySelector(
            ".embedThumbnailLink"
          );
        ...
        case "embedFooterTimestamp":
          ...
          if (!embedObj.footer?.text) hide(embedFooter);
            ...
            display(
              ...
              `
                        ${
                          embedObj.footer.icon_url
                            ? '<img class="embedFooterIcon embedFooterLink" src="' +
                              encodeHTML(url(embedObj.footer.icon_url)) +
                              '">'
                            : ""
      ...
      for (const embedObj of jsonObject.embeds) {
        ...
        const embedElement = embedCont.appendChild(
          embedFragment.firstChild.cloneNode(true)
        );
        ...
        const embedDescription =
          embedElement.querySelector(".embedDescription");
        ...
        const embedThumbnail = embedElement.querySelector(
          ".embedThumbnail > img"
        );
        ...
        if (embedObj.color)
          embedGrid.closest(".embed").style.borderColor =
            typeof embedObj.color === "number"
              ? "#" + embedObj.color.toString(16).padStart(6, "0")
              : embedObj.color;
        else embedGrid.closest(".embed").style.removeProperty("border-color");

        if (embedObj.author?.name)
          display(
            embedAuthor,
            `
                    ${
        ...
        if (
          embedElement.innerText.trim() ||
          embedElement.querySelector(".embedGrid > [style*=display] img")
        )
      ...
      if (
        !multiEmbeds &&
        !embedCont.innerText.trim() &&
        !embedCont.querySelector(".embedGrid > [style*=display] img")
      )
  ...
  editor.on("change", (editor) => {
    ...
    try {
      ...
      if (
        dataKeys.length &&
        !allJsonKeys.some((key) => dataKeys.includes(key))
      ) {
        ...
        if (usedKeys.length > 2)
          return error(
            `'${
              usedKeys[0] +
              "', '" +
              usedKeys.slice(1, usedKeys.length - 1).join("', '")
            }', and '${usedKeys[usedKeys.length - 1]}' are invalid keys.`
          );
        return error(
          `'${
            usedKeys.length == 2
  ...
  picker.on?.("enter", () => {
    const embedIndex =
      multiEmbeds && lastActiveGuiEmbedIndex !== -1
        ? lastActiveGuiEmbedIndex
        : 0;
    if (jsonObject?.embeds[embedIndex]?.color) {
      hexInput.value = jsonObject.embeds[embedIndex].color
        .toString(16)
        .padStart(6, "0");
    ...
    e.addEventListener("click", (el) => {
      const embedIndex =
        multiEmbeds && lastActiveGuiEmbedIndex !== -1
          ? lastActiveGuiEmbedIndex
          : 0;
      const embed = document.querySelectorAll(".msgEmbed .container>.embed")[
        embedIndex
      ];
  ...
  document.querySelector(".opt.json").addEventListener("click", () => {
    ...
    if (emptyEmbedIndex !== -1)
      ...
      return error(
        gui
          .querySelectorAll(".item.guiEmbedName")
          [emptyEmbedIndex].innerText.split(":")[0] + " should not be empty.",
        "3s"
      );
  ...
  document.querySelector(".top-btn.menu")?.addEventListener("click", (e) => {
    if (e.target.closest(".item.dataLink")) {
      const data = encodeJson(json, true).replace(
        /(?<!data=[^=]+|=)=(&|$)/g,
        (x) => (x === "=" ? "" : "&")
      );
      if (!window.chrome)
    ...
    if (e.target.closest(".item.auto")) {
      ...
      if (options.avatar)
        document.querySelector("img.avatar").src = options.avatar;
  ...
  document.body.addEventListener("click", (e) => {
    if (
      ...
      (e.target.classList.contains("top") &&
        colors.classList.contains("display"))
    )
      togglePicker();
  });
  ...
  if (menuMore.parentElement.childElementCount < 1)
    menuMore?.parentElement.classList.add("invisible");
...
Object.defineProperty(window, "json", {
  ...
  set(val) {
    ...
    const embedObjects = val.embeds?.filter(
      (j) => j.constructor === Object && 0 in Object.keys(j)
    );
    // Convert 'embed' to 'embeds' and delete 'embed' or validate and use 'embeds' if provided.
    const embeds = val.embed
      ? [val.embed]
      : embedObjects?.length
      ? embedObjects
      : [];
...
function cleanEmbed(obj, recursing = false) {
  ...
  for (const [key, val] of Object.entries(obj))
    if (val === undefined || val.trim?.() === "")
      ...
      (!Object.keys(val).length && delete obj[key]) ||
        (obj[key] = cleanEmbed(val, true));
    else if (val.constructor === Array)
      // Remove array (val) if it has no keys or recursively remove empty keys from objects in array.
      (!val.length && delete obj[key]) ||
        (obj[key] = val.map((k) => cleanEmbed(k, true)));
